 ...
 *
 * 1、@Autowired: 自动注入,一个组件内部要用到另外一个组件,只需要给用到的那个组件属性上添加一个 @Autowired 注解即可
 *      1> 默认优先按照类型去容器中找对应的组件,找到就赋值
 *              context.getBean(BookService.class);
 *      2> 如果找到多个相同类型的组件,将属性的名称作为组件的id去容器中查找
 *              context.getBean("bookDao");
 *      3> @Qualifier("bookDao"),使用 @Qualifier 明确指定需要装配的组件的id,而不是使用属性名
 ...
 *             不支持@Primary功能,也不支持@Autowired(required = false)
 *      2> @Inject: 首先需要导入javax.inject依赖,和 @Autowired 功能一样,
 *             支持 @Primary,但是不支持 @Autowired(required = false)
 *
 * 3、区别:
 *      最本质的区别:@Autowired 由 Spring 定义的,而 @Resource 和 @Inject 都是Java规范
 *
 ...
 *
 *              // @Bean + 方法参数: 参数从容器中获取
 *              // 这些位置默认不写 @Autowired 效果是一样的都能自动装配
 *              @Bean
 ...
 *              }
 *          注意: 如果组件只有一个有参构造器,这个有参构造器的 @Autowired 可以省略,参数位置的组件还是可以自动从容器中获取
 *       3> [标注在参数上]
 ...
 *    // xxxAware,功能是使用 xxxProcessor 处理器来处理的
