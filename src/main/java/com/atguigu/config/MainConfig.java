 ...
 *
 * @ComponentScan: 包扫描,可以指定多个 @ComponentScan 用来指定不同的扫描策略
 ...
 *      }: 指定扫描的时候只需要包含那些组件
 *      useDefaultFilters = false: 配置includeFilters时,禁用掉默认的规则,因为默认的规则就是扫描所有的,这样只包含才能生效
 *      type扫描类型:
 *          FilterType.ANNOTATION: 按照注解
 *          FilterType.ASSIGNABLE_TYPE: 按照我们给定的类型,只要是我们指定的组件都会被加载到容器中,例如:子类、实现类等等
 *          FilterType.ASPECTJ: 使用AspectJ表达式
 *          FilterType.REGEX: 使用正则表达式
 *          FilterType.CUSTOM: 使用给定的自定义筛选候选项,也可以说是过滤规则,这个类需要实现 TypeFilter 这个接口
 *              Controller.class: 表示标注@Controller注解的类
 *
